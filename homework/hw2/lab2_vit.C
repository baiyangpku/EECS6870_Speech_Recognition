...
double viterbi(const Graph& graph, const matrix<double>& gmmProbs,
    ...
    for (int frameIdx = 1; frameIdx<=frmCnt; frameIdx++){
        for (int stateIdx =0; stateIdx< stateCnt; stateIdx++){
            // find states that can be reached from the start state
            if (chart(frameIdx-1, stateIdx).get_log_prob() > g_zeroLogProb){
               ...
               //  Get arc ID of first outgoing arc.
                ...
                for (int arcIdx = 0; arcIdx < arcCnt; ++arcIdx){
                    Arc arc;
                    // record arcId before its update
                    int oldId = arcId;
                    ...
                    // calculate the probability of going to the destination from this acr
                    double arcProb =  arc.get_log_prob()+ gmmProbs(frameIdx-1, arc.get_gmm());
                    double totalProb = arcProb + chart(frameIdx-1,  stateIdx).get_log_prob();
                    // if this path is more possible than any provious one, update the chart
                    if (totalProb > chart(frameIdx,  dstState).get_log_prob()){
                        chart(frameIdx,  dstState).assign(totalProb, oldId);
                    }
                }
