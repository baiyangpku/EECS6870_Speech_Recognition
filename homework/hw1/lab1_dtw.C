...
double compute_distance(const matrix<double>& matHyp,
    ...
    int Dim = matHyp.size2();
    int testFrame = matHyp.size1();
    int templateFrame = matTempl.size1();
    double DTWmatrix[testFrame][templateFrame];
    //  BEGIN_LAB
    //
    //  Input:
    ...
    for (int i=0; i<testFrame; i++){
        DTWmatrix[i][0] = MAX_double;
    }
        
    for (int j = 0; j < templateFrame; j++){
        DTWmatrix[0][j] = MAX_double;
    }

    double cost = 0;
    DTWmatrix[0][0] = 0;
    for (int i = 1; i < testFrame; i++){
        for(int j = 1; j < templateFrame; j++){
            cost  = 0;
            for(int k=0; k< Dim; k++){
                cost += pow((matHyp(i,k)- matTempl(j,k)),2);
            } 
            DTWmatrix[i][j] = sqrt(cost)+ min(min(DTWmatrix[i-1][j],DTWmatrix[i][j-1]),DTWmatrix[i-1][j-1]);
        }
    }
    //  END_LAB

    return DTWmatrix[testFrame-1][templateFrame-1];
    }
